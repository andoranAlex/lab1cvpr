...
def matchCenter(img, min, max):
    ...
    hsv = cv.cvtColor(img,cv.COLOR_GRAY2BGR)
...
def processAll(list, ethalonImg, min, max):
    ...
    im1 = cv.imread(ethalonImg,0)
    #gray1 = cv.cvtColor(im1, cv.COLOR_RGB2GRAY)
    detector = cv.xfeatures2d_SIFT.create()
    #kps1, descs1 = detector.detectAndCompute(gray1, None)
    pool = ThreadPool(processes=cv.getNumberOfCPUs())
    kps1, descs1 = affine_detect(detector, im1, pool = None)
    print("1 \n")
    for file in list:
        kaze_match(kps1, descs1, im1, file, f, min, max)
        key = cv.waitKey(1)
        if key & 0xFF == ord('q') or key == 27:
            break
if __name__ == '__main__':
    imageList = []
    ethalonImg = 'C:\Users\Oleksii\Pictures\cvprlab1\good\IMG_20181104_111752.JPG'
    for filename in glob.glob('C:\Users\Oleksii\Pictures\cvprlab1\good1\*.jpg'):
            #print("2 \n")
            imageList.append(filename)
    f = open("result.txt", "a")
    #imageList1 = ['C:\Users\Oleksii\Pictures\cvprlab1\good\IMG_20181104_112016_HDR.JPG']
    min = (0, 0, 153)
    max = (255, 51, 255)
    f.write('\n' + "=======police bobik=======" + '\n')
    f.write('\n' + "=======images with obj=======" + '\n')
